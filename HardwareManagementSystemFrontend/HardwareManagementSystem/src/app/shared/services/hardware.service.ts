...
export class HardwareService {
  ...
  getHwTypes(): Observable<HWType[]> {
    return this.http.get<HWType[]>(this.hwTypesUrl, {
      headers: this.getAuthHeaders(),
    });
  }

  getHwDetails(): Observable<HWDetails[]> {
    return this.http.get<HWDetails[]>(this.hwDetailsUrl, {
      headers: this.getAuthHeaders(),
    });
  }

  addHwDetails(hwDetails: HWDetails): Observable<HWDetails> {
    return this.http.post<HWDetails>(this.hwDetailsUrl, hwDetails, {
      headers: this.getAuthHeaders(),
    });
  }

  updateHwDetails(hwDetails: HWDetails): Observable<HWDetails> {
    return this.http.put<HWDetails>(
      `${this.hwDetailsUrl}/${hwDetails.id}`,
      hwDetails,
      { headers: this.getAuthHeaders() }
    );
  }
  ...
  deleteHwDetails(id: number): Observable<void> {
    return this.http.delete<void>(`${this.hwDetailsUrl}/${id}`, {
      headers: this.getAuthHeaders(),
    });
  }
  ...
  returnMultipleHardware(hardwareIds: string[]): Observable<void> {
    return this.http.post<void>(`${this.hwDetailsUrl}/returnMultiple`, {
      hardwareIds,
    });
  }
  // Get hardware details by ID ///api/hw-details
  getHardwareById(hardwareId: string): Observable<any> {
    return this.http.get<any>(`${this.hwDetailsUrl}/${hardwareId}`, {
      headers: this.getAuthHeaders(),
    });
  }
